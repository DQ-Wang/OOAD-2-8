...
public interface ServiceOrderMapper {
    @Insert("INSERT INTO service_order (" +
            ...
            "create_time, problem_image_url, description, product_id, express_id, customer_id " + // 新增：关联商铺ID和售后单ID（实验核心关联字段）
            ") VALUES (" +
            "#{serviceSn}, #{serviceConsignee}, #{serviceMobile}, #{address}, #{type}, #{status}, " +
            "#{createTime}, #{problemImageUrl}, #{description}, #{productId}, #{expressId}, #{customerId} " + // 对应新增字段的参数占位符
            ")")
